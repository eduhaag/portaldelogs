...
class ProgressProfilerParser:
    ...
    def _parse_session_header(self, cursor: int) -> Tuple[ProfileSession, int]:
        ...
        if len(tokens) < 5 or not self._is_int(tokens[0]):
    ...
    def _parse_sources(self, cursor: int) -> int:
        while cursor < len(self.lines):
            ...
            srcid = int(tokens[0])
            srcname = tokens[1]
            listname = tokens[2] if len(tokens) >= 4 else ""
            crc_val = int(tokens[-1]) if self._is_int(tokens[-1]) else 0
    ...
    def _looks_like_source(self, tokens: List[str]) -> bool:
        return (
            ...
            and not all(self._is_int(token) for token in tokens[:4])
        )
