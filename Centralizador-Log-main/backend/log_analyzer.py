...
class LogAnalyzer:
    ...
    def __init__(self):
        ...
        self.default_patterns = [
            ...
            # Progress/OpenEdge
            r"PROGRESS", r"OpenEdge", r"Progress Database", r"4GL",
    ...
    def extract_timestamp(self, line: str) -> str:
        ...
        # PRIORIDADE 1: Timestamp Progress (mais específico e preciso)
        timestamp_patterns = [
            r'\[(\d{2}/\d{2}/\d{2}@\d{2}:\d{2}:\d{2}\.\d{3}[+-]\d{4})\]',  # [25/11/24@10:45:34.525-0300] - Progress
    ...
    def extract_log_message(self, line: str) -> str:
        ...
        progress_pattern = r'^\[[\d/]+@[\d:.-]+\]\s+P-\d+\s+T-\d+\s+(\d+)\s+'
        ...
        other_metadata_patterns = [
            # [2024-09-25 10:45:34] [INFO] [Thread-123]
            r'^\[\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\]\s*\[[^\]]+\]\s*\[[^\]]+\]\s*',
            # 2024-09-25 10:45:34.123 INFO [main] 
            r'^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}[.\d]*\s+\w+\s+\[[^\]]+\]\s*',
            # Sep 25 10:45:34 hostname processname[1234]:
            r'^\w{3}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+\w+\s+\w+\[\d+\]:\s*',
            # 10:45:34.525 [Thread-123] INFO
            r'^\d{2}:\d{2}:\d{2}[.\d]*\s+\[[^\]]+\]\s+\w+\s*',
            # Timestamp simples no início: 2024-01-01 12:00:00
            r'^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s*',
            # Timestamp formato brasileiro: 25/09/2024 10:45:34
            r'^\d{2}/\d{2}/\d{4}\s+\d{2}:\d{2}:\d{2}\s*',
        ]
        
        for pattern in other_metadata_patterns:
    ...
    def _detect_log_type(self, log_content: str, sample_size: int = 200) -> str:
        """
        Detecta o tipo de log baseado no conteúdo.
        
        Args:
            log_content: Conteúdo do log
            sample_size: Número de linhas a analisar (padrão: 200)
            
        Returns:
            "Datasul", "LOGIX" ou "Other"
        """
        lines = log_content.split('\n')[:sample_size]
        sample_text = '\n'.join(lines).lower()
        
        # Palavras-chave Progress/Datasul/PASOE/AppServer (muito específicas)
        datasul_keywords = [
            'datasul', 'progress', 'openedge', '4gl', 'propath', 
            # PASOE específico
            'pasoe', 'pasoeclient', 'pdsoe', 'webhandler', 'abl websocket',
            'catalina', 'tomcat', 'msagent', 'oepas', 'pas for openedge',
            # AppServer específico
            'appserver', 'appsvr', 'app server', '_mprosrv', 'nameserver',
            'appservice', 'broker', 'srv -S', 'agent process',
            # Padrões gerais Progress
            'ft[0-9]{4}', 'log:manager', 'p-[0-9]{6}', 't-[0-9]{6}',
            '.p', '.r', '.w', '(procedure', 'promsgs', '_progres'
        ]
        
        # Palavras-chave LOGIX (muito específicas)
        logix_keywords = [
            'logix', 'totvs', 'framework', 'frw:', 'nfe:', 'danfe', 
            'sefaz', 'log4j', 'schema xml', 'prd', 'entidade',
            'validação de schema', 'xml.nfe', 'totvs - frw'
        ]
        ...
        for keyword in datasul_keywords:
            if keyword.startswith('[') and keyword.endswith(']'):
                # É regex
                import re
                datasul_score += len(re.findall(keyword, sample_text, re.IGNORECASE))
            else:
                datasul_score += sample_text.count(keyword)
        
        logix_score = 0
        for keyword in logix_keywords:
            logix_score += sample_text.count(keyword)
        
        # Detectar timestamp Progress específico [DD/MM/YY@HH:MM:SS]
        import re
        progress_timestamp_pattern = r'\[\d{2}/\d{2}/\d{2}@\d{2}:\d{2}:\d{2}'
        if re.search(progress_timestamp_pattern, sample_text):
            datasul_score += 5  # Peso alto para timestamp Progress
        
        # NOVO: Detectar padrões específicos de PASOE
        pasoe_patterns = [
            r'SEVERE:.*Exception',  # PASOE Catalina logs
            r'WebHandler',
            r'ABLWebApp',
            r'oeablSecurity',
            r'msagent\s+\d+',
        ...
        for pattern in pasoe_patterns:
            if re.search(pattern, sample_text, re.IGNORECASE):
                datasul_score += 2  # Peso médio para padrões PASOE
        
        # NOVO: Detectar padrões específicos de AppServer
        appserver_patterns = [
            r'Broker.*pid\s+\d+',
            r'Agent.*started',
            r'_mprosrv',
            r'nameserver.*port',
            r'AppServer.*connected',
            r'srv\s+-S\s+\d+'
        ]
        for pattern in appserver_patterns:
            if re.search(pattern, sample_text, re.IGNORECASE):
                datasul_score += 2  # Peso médio para padrões AppServer
        
        # Decisão baseada em scores
        if logix_score >= 3:
            logger.info(f"Log type detected: LOGIX (score: {logix_score})")
            return "LOGIX"
        elif datasul_score >= 3:
            logger.info(f"Log type detected: Datasul (score: {datasul_score})")
            return "Datasul"
        else:
            logger.info(f"Log type detected: Other (Datasul: {datasul_score}, LOGIX: {logix_score})")
            return "Other"
    ...
    def analyze_log_content(
        ...
        try:
            # NOVO: Detectar tipo de log automaticamente
            ...
            # Adicionar padrões Datasul se for tipo Datasul
            if log_type == "Datasul" or log_type == "Other":
            ...
            for line_num, line in enumerate(lines, start=1):
                ...
                # NOVO: Ignorar ruído Progress (heartbeat/conexão)
                ...
                if self._is_attention_point(clean_message):
                    ...
                    attention_item = {
                        ...
                        "matched_keywords": [pattern for pattern in self.attention_patterns 
                ...
                is_datasul_error = self._is_datasul_error(clean_message)  # NOVO: Datasul busca em TODOS os logs
                is_logix_error = self._is_logix_error(clean_message) if log_type == "LOGIX" else False  # Só busca LOGIX em logs LOGIX
                ...
                if is_main_error or is_datasul_error or is_logix_error or is_structured_error:
                    ...
                    if self.totvs_loader:
                        ...
                        if totvs_info:
                            ...
                            # Atualizar tipo do erro para "TOTVS" quando encontrado
            ...
            new_errors_analysis = self._detect_new_errors(lines, regex)
            
            # Análise de performance
            performance_analysis = self.analyze_performance(lines, log_type)
            ...
            result = {
                ...
                "top_programs_methods": self._analyze_callers_and_programs(lines, log_type),
                "informational_lines": self._detect_informational_lines(lines),
    ...
    async def load_custom_patterns_from_db(self, db):
        ...
        try:
            ...
            fallback_patterns = list_records("custom_patterns", {"active": True}, sort_field="created_at", descending=True, limit=1000)
