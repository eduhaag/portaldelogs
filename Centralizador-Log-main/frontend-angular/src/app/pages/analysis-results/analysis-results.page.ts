...
export class AnalysisResultsPageComponent implements OnInit {
    ...
    protected readonly potentialErrorColumns: PoTableColumn[] = [
    ...
    protected readonly patternSuggestionColumns: PoTableColumn[] = [
    ...
    ngOnInit(): void {
        ...
        this.analysisResult = sessionData.result;
        this.analyzedFilename = sessionData.filename;
        this.analyzedAt = sessionData.analyzedAt;
        this.resultRows = (sessionData.result.results ?? []).map((item) => this.mapResultRow(item));
        this.summaryCards = this.buildSummaryCards(sessionData.result);
        this.highlightedStats = this.buildHighlightedStats(sessionData.result);
        this.attentionPoints = sessionData.result.attention_points ?? [];
        this.informationalLines = sessionData.result.informational_lines ?? [];
        this.topProgramRows = sessionData.result.top_programs_methods?.top_programs ?? [];
        this.buildBackendSummaryBlocks(sessionData.result);
        this.buildNewErrorsBlocks(sessionData.result);
        this.buildStructuredBlocks(sessionData.result);
        this.buildPerformanceBlocks(sessionData.result);
    }
