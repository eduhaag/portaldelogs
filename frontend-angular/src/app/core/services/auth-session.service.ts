...
export class AuthSessionService {
    ...
    private getSession(): AuthUserSession | null {
        ...
        try {
            ...
            if (!this.hasEssentialSessionData(parsed) || this.isSessionExpired(parsed)) {
                this.logout();
                return null;
            }
    ...
    private hasEssentialSessionData(session: AuthUserSession): boolean {
        return !!session.token?.trim() && !!session.username?.trim() && !!session.displayName?.trim();
    }
